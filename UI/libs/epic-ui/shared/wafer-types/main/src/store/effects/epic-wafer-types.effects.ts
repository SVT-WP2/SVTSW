...
import { EpicWaferTypesSelectors } from '../selectors'
...
@Injectable()
export class EpicWaferTypesEffects {

    protected readonly store = inject(Store)
    protected readonly actions$ = inject(Actions)
    protected readonly epicWaferTopographiesApiClient = inject(EpicWaferTypesApiClient)
            ...
            .pipe(
                ...
                mergeMap(([{ force }, isAllDataFetched]) => {
                    if (!force && isAllDataFetched) {
                        return this.store.pipe(select(StoreSelectors.selectAllWaferTypes))
                            .pipe(
                                take(1),
                    ...
                    return this.epicWaferTopographiesApiClient.fetchAll()
                        .pipe(
                            mapResponse({
                                next: (entities) => (
    ...
    protected readonly createRequest$ = createEffect(() =>
        this.actions$
            .pipe(
                ...
                mergeMap(({ create }) => {
                    return this.epicWaferTopographiesApiClient.create(create)
                        .pipe(
                            mapResponse({
                                next: (entity) => (
    ...
    protected readonly updateRequest$ = createEffect(() =>
        this.actions$
            .pipe(
                ...
                mergeMap(({ id, update }) => {
                    return this.epicWaferTopographiesApiClient.update(id, update)
                        .pipe(
                            mapResponse({
                                next: (entity) => (
