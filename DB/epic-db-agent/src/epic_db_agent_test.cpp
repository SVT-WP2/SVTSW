...
int main()
{
  logger.logInfo("********************** Testing Epic Db Agent, version:" +
                     version,
...
void test_getVersions()
{
  std::vector<EpicDbInterface::dbVersion> dbVersion_v;

  try
  {
    int nDbVerisons = EpicDbInterface::getAllVersions(dbVersion_v);
...
void test_addEnumValues()
{
  EpicDbInterface::addEnumValue("test.enum_waferTech", "TPSCo65");
  EpicDbInterface::addEnumValue("test.enum_waferType", "MOSS");

  //! get enum values for enum_engineeringRun
  std::vector<std::string> enum_waferTech_values =
      EpicDbInterface::getAllEnumValues("test.enum_waferTech");
  ...
  std::vector<std::string> enum_waferType_values =
      EpicDbInterface::getAllEnumValues("test.enum_waferType");
...
{
  std::array<EpicDbInterface::dbWaferRecords, 2> wafers = {
...
void test_getWafers()
{
  std::vector<EpicDbInterface::dbWaferRecords> wafers_v;
  try
  {
    int nDbWafers = EpicDbInterface::getAllWafers(wafers_v);
