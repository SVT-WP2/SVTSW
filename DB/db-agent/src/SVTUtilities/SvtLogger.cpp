  ...
  , msgId_(0U)
{
  ...
  char *logFilePath = getenv("SVT_LOG_FILE");
  ...
  char *logVer = getenv("SVT_LOG_VERBOSITY");
  if (logVer != nullptr)
  {
    if (std::string(logVer).find("STANDARD") != std::string::npos)
      logVerbosity_ = Mode::STANDARD;
    else if (std::string(logVer).find("DEBUG") != std::string::npos)
      logVerbosity_ = Mode::DEBUG;
...
SvtLogger &SvtLogger::getInstance()
{
  static SvtLogger pinstance;
  return pinstance;
}
