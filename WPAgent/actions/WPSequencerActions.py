...
def run_sequencer(filepath, user=None, waferAgentName=None, executor=None):
    ...
    try:
        ...
        for i, step in enumerate(sequence, 1):
            ...
            if not result:
                ...
                return ResponseBuilder.error(
                    "RunSequencerReply", f"Step {i} failed: {error_msg}", 500
                )
        ...
        return ResponseBuilder.success(
            "RunSequencerReply", f"Successfully executed {len(sequence)} commands"
        )

    except FileNotFoundError:
        return ResponseBuilder.error(
            "RunSequencerReply", f"File not found: {filepath}", 404
        )

    except json.JSONDecodeError as e:
        return ResponseBuilder.error(
            "RunSequencerReply", f"Invalid JSON: {str(e)}", 400
        )
        ...
        return ResponseBuilder.error(
            "RunSequencerReply", f"Sequencer error: {str(e)}", 500
        )
