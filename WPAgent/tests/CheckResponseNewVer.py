...
def test_all_commands():
    ...
    commands_to_test = [
        ...
        (
            "MoveChuckXY",
            lambda: __import__(
                "actions.WPTestingActions", fromlist=["move_chuck_xy"]
            ).move_chuck_xy(x=10, y=20),
        ),
        (
            "MoveChuckZ",
            lambda: __import__(
                "actions.WPTestingActions", fromlist=["move_chuck_z"]
            ).move_chuck_z(z=5),
        ),
        (
            "RunPTPA",
            lambda: __import__(
                "actions.WPTestingActions", fromlist=["run_ptpa"]
            ).run_ptpa(),
        ),
        (
            "StepNextDie",
            lambda: __import__(
                "actions.WPTestingActions", fromlist=["step_next_die"]
            ).move_chuck_next_die(),
        ),
        (
            "GoToDie",
            lambda: __import__(
                "actions.WPTestingActions", fromlist=["go_to_die"]
            ).move_chuck_row_column(col=1, row=1),
        ),
        (
            "OpenProject",
            lambda: __import__(
                "actions.WPTestingActions", fromlist=["open_project"]
            ).open_project(),
        ),
        (
            "FindHome",
            lambda: __import__(
                "actions.WPTestingActions", fromlist=["find_home"]
            ).find_home(),
        ),
        (
            "SwitchCamera",
            lambda: __import__(
                "actions.WPTestingActions", fromlist=["switch_camera"]
            ).switch_camera(mount_point="Top"),
        ),
        (
            "MoveChuckHome",
            lambda: __import__(
                "actions.WPTestingActions", fromlist=["move_chuck_home"]
            ).move_chuck_home(),
        ),
        (
            "Unload",
            lambda: __import__(
                "actions.WPTestingActions", fromlist=["unload_wafer"]
            ).unload_wafer(),
        ),
        (
            "Cleaning",
            lambda: __import__(
                "actions.WPTestingActions", fromlist=["clean_probe_station"]
            ).clean_probe_station(),
        ),
        (
            "AlignWafer",
            lambda: __import__(
                "actions.WPTestingActions", fromlist=["align_wafer"]
            ).align_wafer(align_die_col=1, align_die_row=1),
        ),
        (
            "GoToContact",
            lambda: __import__(
                "actions.WPTestingActions", fromlist=["go_to_contact"]
            ).move_chuck_contact(),
        ),
        (
            "GoToSeparation",
            lambda: __import__(
                "actions.WPTestingActions", fromlist=["go_to_separation"]
            ).Move_chuck_separation(),
        ),
        (
            "AutoFocus",
            lambda: __import__(
                "actions.WPTestingActions", fromlist=["auto_focus"]
            ).auto_focus(),
        ),
        (
            "Load",
            lambda: __import__(
                "actions.WPTestingActions", fromlist=["load_wafer"]
            ).load_wafer(),
        ),
        (
            "MoveChuckToWorkArea",
            lambda: __import__(
                "actions.WPTestingActions", fromlist=["move_chuck_work_area"]
            ).move_chuck_work_area(work_area=0),
        ),
        (
            "LocalMode",
            lambda: __import__(
                "actions.WPTestingActions", fromlist=["local_state"]
            ).local_mode(),
        ),
        (
            "GoToPreviousDie",
            lambda: __import__(
                "actions.WPTestingActions", fromlist=["go_to_previous_die"]
            ).move_chuck_previous_die(),
        ),
        (
            "GetChuckPosition",
            lambda: __import__(
                "actions.WPTestingActions", fromlist=["get_chuck_position"]
            ).get_chuck_position(),
        ),
        # Project Actions
        (
            "ShowProjectStatus",
            lambda: __import__(
                "actions.WPProjectActions", fromlist=["get_project_status"]
            ).show_status(),
        ),
        (
            "GetInfo",
            lambda: __import__(
                "actions.WPProjectActions", fromlist=["get_info"]
            ).get_info(),
        ),
        (
            "help",
            lambda: __import__(
                "actions.WPProjectActions", fromlist=["help_command"]
            ).help(),
        ),
        (
            "ResetAgent",
            lambda: __import__(
                "actions.WPProjectActions", fromlist=["reset_agent_state"]
            ).reset_agent(),
        ),
        (
            "GetAgentState",
            lambda: __import__(
                "actions.WPProjectActions", fromlist=["get_agent_state"]
            ).get_agent_state(),
        ),
        # Database Actions
        (
            "ListProbers",
            lambda: __import__(
                "actions.WPDataBaseActions", fromlist=["list_probers"]
            ).list_probers(),
        ),
        (
            "ListChipTypes",
            lambda: __import__(
                "actions.WPDataBaseActions", fromlist=["list_chip_types"]
            ).list_chip_types(),
        ),
        # Command Actions
        (
            ...
            lambda: __import__(
                "actions.WPCommandActions", fromlist=["list_available_commands"]
            ).list_available_commands({}),
        ),
    ]

    results = {"new_format": [], "old_format": [], "mixed_format": [], "error": []}
