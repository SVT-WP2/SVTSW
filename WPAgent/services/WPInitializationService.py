...
class WPInitializationService:
    ...
    def initialize_from_database(self, force=False, db_timeout=15.0):
        ...
        try:
            ...
            if not asicFamily:
                return {"status": "error", "output": "ASIC family is required"}
            ...
            orientation = input(
                "\nEnter wafer orientation (e.g., East, West, South, North): "
            ).strip()

            if not orientation:
                return {"status": "error", "output": "Orientation is required"}
            ...
            for project in all_projects:
                ...
                if (
                    str(project.get("orientation", "")).lower()
            ...
            machine_projects = [
                p for p in all_projects if p.get("wpMachineId") == selected_machineId
            ]
            family_projects = [
                p
            ...
            print(
                f"🔽 Filtered by prober ID {selected_machineId}: {len(machine_projects)} projects"
            )
            print(
                f"🔽 Filtered by ASIC family '{asicFamily}': {len(family_projects)} projects"
            )
            print(
                f"🔽 Filtered by orientation '{orientation}°': {len(filtered_projects)} projects"
            )
            ...
            alignmentDie_str = self._format_die_position(
                selected_project.get("alignmentDie")
            )
    ...
    def _select_machine_from_list(self, machines):
        ...
        while True:
            try:
                selection = input(
                    f"\nSelect probe machine (1-{len(machines)}) or 'q' to quit: "
                ).strip()
